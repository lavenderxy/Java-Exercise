...
public class ListItem {
	...
	public ListItem stretch(int n) {
		...
		int []number=new int[this.length()];
		...
		int len=number[number.length-1];
		ListItem b=new ListItem(len,null);
		for (int i=0;i<number.length;i++){
			int len1=number[number.length-1-i];
			for (int j=0;j<n;j++){
				if (!(i==0 && j==0)){
					b=new ListItem(len1,b);
				}
			}
		}
